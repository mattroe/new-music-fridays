...
- **Durable run history is opt-in via email, not commits.** `runs/<date>/` is ephemeral on the cloud VM, so the per-run *reasoning* (kept/skipped candidates) survives only in the transcript unless `delivery.yaml::send_run_log` is `true` — which emails a redacted `[NMF log]` copy via the same `scripts/send-email.mjs`. Email is the mechanism (rather than committing a log back to the repo) so a fork needs no GitHub push access and never publishes its listening history to a possibly-public repo. The send is best-effort: a failure is noted but never blocks or fails the digest. See [#17](https://github.com/mattroe/new-music-fridays/issues/17).
