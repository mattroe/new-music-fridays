...
- `config/review-sources.yaml` — endorsement signals and the citation allowlist; endorsements **weight ranking** and gate Worth a Second Look, but are **never rendered in the email** (no scores/citations shown)
...
- Filter to listening-profile fit (same genre profile as above). **Maximum 2 picks.** Each pick **must carry at least one endorsement** (a valid `citation_formats` string) — that acclaim is the *internal selection gate* for this section (it's what makes a prior-week release "worth a second look"); if it has none, omit it. **The endorsement is not rendered** — the email line is taste-fit prose, no score (see *Compose*). An empty result is fine — better than a weak pick.
