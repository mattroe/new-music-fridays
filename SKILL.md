...
Write the complete summary as an email using the `resend` connector. Send **to**: mroe@hey.com. Send **from**: claude@emails.mroe.net — pass this address as a plain email string with no display-name wrapper (the `from` field does not accept "Name <email>" format). Subject line: "New Music Friday - MM-DD-YYYY" where the date field is actually today's date.