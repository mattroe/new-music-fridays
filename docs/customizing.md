...
- `config/musicbrainz.yaml` — MusicBrainz verification. After discovery, each kept candidate is resolved against the open [MusicBrainz](https://musicbrainz.org) database to confirm it exists and check its release-group first-release-date — confirming genuinely-new releases, demoting reissues, and flagging picks too new to have an entry yet (kept, never dropped). It's a curation *signal*, not a filter, and never a citation source. `enabled` is the on/off switch (default on; set `false` to disable); `min_score` is the match-score floor. **One setup step:** add `musicbrainz.org` to your routine's Custom Network-access allowlist (no secret needed). Until you do, the step harmlessly no-ops and runs proceed exactly as before.
