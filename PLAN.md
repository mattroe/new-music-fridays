...
The routine itself works end-to-end (last run sent successfully from `claude@emails.mroe.net` to `mroe@hey.com`). Phase 2 is purely a refactor for maintainability — **do not change observable behavior**.
...
- **Delivery details** in line ~30: from address (`claude@emails.mroe.net`), to address (`mroe@hey.com`), subject format (`New Music Friday - MM-DD-YYYY`), and the "plain email string, no display-name wrapper" constraint on the `from` field → `config/delivery.yaml`
