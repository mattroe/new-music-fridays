...
5. Composes a digest — Top 5 picks, Section A (up to 10 known-artist releases), Section B (up to 5 discovery picks), Worth a Second Look, and one "Off the Beaten Path" horizon pick (a deliberate one-genre-hop-out stretch to expand your horizons, gated on acclaim + a bridge to your taste; empty on a thin week) — with sections mutually exclusive so no pick repeats; ranking is by taste-fit (endorsements weight it but are never shown)
...
- `scripts/musicbrainz.mjs` — resolves kept candidates against the MusicBrainz API to verify they exist and read each release-group's first-release-date, and optionally enrich them with the authoritative label and distilled personnel credits; also enumerates releases by artists you already listen to, powering the coverage-gap sweep that rescues in-window releases the editorial sweep missed (zero-dependency; fail-soft; one hardcoded host)
